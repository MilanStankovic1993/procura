...
use App\Http\Requests\Api\V1\Analyses\ConfirmAnalysisCostsRequest;
...
class ConfirmAnalysisCostsController extends Controller
{
    public function __invoke(
        ConfirmAnalysisCostsRequest $request,
        string $analysis,
        OrganizationContext $context,
        ConfirmAnalysisCosts $confirmCosts,
    ): JsonResponse {
        $record = Analysis::query()
            ->forOrganization($context->organization())
            ->findOrFail($analysis);
        Gate::authorize('submit', $record);
        $result = $confirmCosts->confirm(
