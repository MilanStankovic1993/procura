...
final class StoreSellComparableMarketNormalizationRequest extends FormRequest
{
    protected function prepareForValidation(): void
    {
        $this->merge([
            'target_country_code' => strtoupper(
                trim((string) $this->input('target_country_code', '')),
            ),
            'target_currency_code' => strtoupper(
                trim((string) $this->input('target_currency_code', '')),
            ),
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'evidence_reference' => ['required', 'string', 'max:2048'],
            'compatibility_note' => ['required', 'string', 'max:5000'],
